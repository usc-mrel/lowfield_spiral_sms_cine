function step = line_search_tric(old, update, Data, para)
%--------------------------------------------------------------------------
...
%      fprintf(['Iter = ' num2str(i) '... '])
    ...
    new_ = repmat(new, [1, 1, 1, 3]);
    new_ = permute(new_, [1, 2, 4, 3, 5]);
    new_ = reshape(new_, [para.Recon.image_size, nframe * 3, 1, nsms]);
    
    fidelity_new = compute_fidelity_for_line_search_yt(new_, Data, para);
