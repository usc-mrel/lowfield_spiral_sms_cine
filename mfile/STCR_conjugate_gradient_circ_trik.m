function [Image,para] = STCR_conjugate_gradient_circ_trik(Data,para)
%--------------------------------------------------------------------------
...
nsms = size(new_img_x, 5);
nframe = size(new_img_x, 3);
...
%% update term (gradient of fidelity, temporal/spatial TV)
    ...
    tic; 
    new_img_x_ = repmat(new_img_x, [1, 1, 1, 3]);
    new_img_x_ = permute(new_img_x_, [1, 2, 4, 3, 5]);
    new_img_x_ = reshape(new_img_x_, [para.Recon.image_size, nframe * 3, 1, nsms]);
    
    [update_term,fidelity_norm] = fidelity(new_img_x_);
    
    update_term = reshape(update_term, [para.Recon.image_size, 3, nframe, nsms]);
    update_term = sum(update_term, 3);
    update_term = permute(update_term, [1, 2, 4, 3, 5]);
...
%% line search    
    ...
    step_size = line_search_tric(new_img_x,update_term_old,Data,para);
